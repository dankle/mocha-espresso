...
Rerun.reRunMain = function(xmlName, jsonName, rerun, rerunCount, mochaTestFolder, reportDir, timeout, callback) {
  ...
  createFinalReportFiles(jsonName, function() {
    if(rerun) {
      ...
      reRun(xmlName, jsonName, jsonName, 0, false, timeout, function(rerunFailed, finalReportName) {
...
// Rerun main function
var reRun = function(xmlName, jsonName, originalFileName, loop, rerunFailed, timeout, callback) {
  ...
  if (loop < rerunTimes) {
    ...
    if (failedTestsArray.length !== 0) {
      ...
      var mochaTimeout = timeout || '--timeout 900000000';
      ...
      child.on('close', function(code) {
        ...
        reRun(xmlName, jsonName, originalFileName, loop+1, rerunFailed, timeout, callback);
      });
    } else {
      reRun(xmlName, jsonName, originalFileName, loop+1, rerunFailed, timeout, callback);
    }
