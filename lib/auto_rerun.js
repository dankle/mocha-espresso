...
var getFailedTestIds = function (jsonFileName) {
  ...
  var json_file_name = jsonFileName;
  var reportPath = Path.join(report_directory, json_file_name);
  var report_json = JSON.parse(Fs.readFileSync(reportPath, 'utf8'));
  var failed_case_ids = [];
  var tc_id;
...
var updateXmlReport = function(xmlFileName, jsonFileName){
  ...
  var xml_name = xmlFileName;
  var xml_path = Path.join(report_directory, xml_name);
  ...
  var json_name = jsonFileName;
  var json_path = Path.join(report_directory, json_name);
...
var mergeJsonReport = function(oldJsonFileName, jsonFileName, callback) {
  ...
  var json_path = Path.join(report_directory, json_name);
  var oldJson = JSON.parse(Fs.readFileSync(json_path, 'utf8'));

  // Get new Mocha json report file
  json_name = jsonFileName;
  json_path = Path.join(report_directory, json_name);
  var json = JSON.parse(Fs.readFileSync(json_path, 'utf8'));
  ...
  json_path = Path.join(report_directory, 'mocha_report_final.json');
...
var createFinalReportFiles = function(firstJsonReport, callback) {
  ...
  var final_json_report = Path.join(report_directory, 'mocha_report_final.json');

  Fs.stat(final_json_report, function(err, stats) {
    if(err) {
      var first_json_report = Path.join(report_directory, firstJsonReport);
...
Rerun.reRunMain = function(xml_name, json_name, rerun, rerun_count, mocha_test_folder, global_mocha_bin, report_dir, callback) {
  report_directory = report_dir;
  createFinalReportFiles(json_name, function() {
    if(rerun) {
      rerun_times = rerun_count;
      run_path = mocha_test_folder;
      reRun(xml_name, json_name, json_name, global_mocha_bin, 0, false, function(rerunFailed, finalReportName) {
        return callback(rerunFailed, finalReportName);
      });
    } else {
      return callback(false, 'mocha_report_final.json');
    }
  });
};

// Rerun main function
var reRun = function(xml_name, json_name, originalFileName, global_mocha_bin, loop, rerunFailed, callback) {
  var finalReportName = 'mocha_report_final.json';
  ...
  if (loop < rerun_times) {
    ...
    if (failedTestsArray.length !== 0) {
      ...
      var rerun_mocha_report_file = Path.join(report_directory, rerun_mocha_report_file_name);

      var reRunCmd = global_mocha_bin + ' --no-colors -R json --recursive --slow 5000 ' +
          '--timeout 900000000' + run_path + ' -g ' +
        '\'' + failedTests + '\' > ' + rerun_mocha_report_file;

      var child = Exec(reRunCmd);
      child.stderr.on('data', function(data) {
        console.log('Error when running: ' + reRunCmd + ' : ' + data);
        rerunFailed = true;
      });
      child.on('error', function(err) {
        console.log('Error when running: ' + reRunCmd + ' : ' + err);
        rerunFailed = true;
      });
      child.on('close', function(code) {
        ...
        updateJsonReport(finalReportName, rerun_mocha_report_file_name);
        json_name = rerun_mocha_report_file_name;
        reRun(xml_name, json_name, originalFileName, global_mocha_bin, loop+1, rerunFailed, callback);
      });
    } else {
      reRun(xml_name, json_name, originalFileName, global_mocha_bin, loop+1, rerunFailed, callback);
    }
