...
var getFailedTestIds = function (jsonFileName) {
  ...
  var report_json = require(Path.join(Config.report_directory, json_file_name));
  var failed_case_ids = [];
  var tc_id;
...
var updateXmlReport = function(xmlFileName, jsonFileName){
  ...
  var xml_path = Path.join(Config.report_directory, xml_name);
  var xml = Fs.readFileSync(xml_path, 'utf8');
  var xmlDoc = Libxmljs.parseXmlString(xml);

  // Get Mocha json report file
  var json_name = jsonFileName || 'mocha_report.json';
  var json_path = Path.join(Config.report_directory, json_name);
...
var mergeJsonReport = function(oldJsonFileName, jsonFileName, callback) {
  ...
  json_path = Path.join(Config.report_directory, json_name);
  var json = require(json_path);
  ...
  json_path = Path.join(Config.report_directory, 'mocha_report_final.json');
...
var createFinalReportFiles = function(firstJsonReport, callback) {
  ...
  var final_json_report = Path.join(Config.report_directory, 'mocha_report_final.json');

  Fs.stat(final_json_report, function(err, stats) {
    if(err) {
      var first_json_report = Path.join(Config.report_directory, firstJsonReport);
...
var reRun = function(xml_name, json_name, originalFileName, loop, rerunFailed, callback) {
  ...
  if (loop < rerun_times) {
    ...
    if (failedTestsArray.length !== 0) {
      ...
      var rerun_mocha_report_file = Path.join(Config.report_directory, rerun_mocha_report_file_name);
