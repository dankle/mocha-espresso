...
var mergeJsonReport = function(oldJsonFileName, jsonFileName, callback) {
  ...
  jsonPath = Path.join(reportDirectory, MOCHA_FINAL_REPORT_NAME);
  ...
  Fs.stat(finalJsonReportPath, function(err, stats) {
    if(err) {
      ...
      mergeJsonReport(MOCHA_FINAL_REPORT_NAME, firstJsonReportName, function(err) {
        if(err){
          console.log('Error when merging final json report with ' + firstJsonReportName);
        }
      });
    }
    return callback();
...
Rerun.reRunMain = function(xmlName, jsonName, rerun, rerunCount, mochaTestFolder, reportDir, callback) {
  reportDirectory = reportDir;
  createFinalReportFiles(jsonName, function() {
    if(rerun) {
      rerunTimes = rerunCount;
      mochaRunPath = mochaTestFolder;
      reRun(xmlName, jsonName, jsonName, 0, false, function(rerunFailed, finalReportName) {
...
// Rerun main function
var reRun = function(xmlName, jsonName, originalFileName, loop, rerunFailed, callback) {
  var finalReportName = MOCHA_FINAL_REPORT_NAME;

  if (loop < rerunTimes) {
    var failedTestsArray = getFailedTestIds(jsonName);
    if (failedTestsArray.length !== 0) {
      var failedTests = failedTestsArray.join('|');
      var rerunMochaReportFileName = 'rerun-' + loop.toString() + '_' + originalFileName;
      ...
      var reRunCmd = 'node_modules/.bin/mocha' + ' --no-colors -R json --recursive --slow 5000 ' +
          '--timeout 900000000' + mochaRunPath + ' -g ' +
      ...
      child.on('close', function(code) {
        ...
        updateXmlReport(xmlName, rerunMochaReportFileName);
        updateJsonReport(finalReportName, rerunMochaReportFileName);
        updateJsonReport(originalFileName, rerunMochaReportFileName);
        jsonName = rerunMochaReportFileName;
        reRun(xmlName, jsonName, originalFileName, loop+1, rerunFailed, callback);
      });
    } else {
      reRun(xmlName, jsonName, originalFileName, loop+1, rerunFailed, callback);
    }
