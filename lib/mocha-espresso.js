...
var runMochaProcess = function(file) {
  ...
  var mochaCommand = multiArgument + ' ' + 'mocha ' + mochaArguments +
      ' ' + file;
  ...
  child.on('close', function(code) {
    ...
    if(code > 0) {
      ...
      Rerun.reRunMain(outputReportPath + '.xml', outputReportPath + '.json', true, rerun_times, mochaTestFolder, reportDirectory, function(rerunFailed, finalReportName){
        if(!debug) bar.tick(1);
        handleResult(rerunFailed, commentOnGithub, finalReportName);
      });
    } else {
      Rerun.reRunMain(outputReportPath + '.xml', outputReportPath + '.json', false, 0, mochaTestFolder, reportDirectory, function(rerunFailed, finalReportName) {
        if (!debug) bar.tick(1);
        handleResult(rerunFailed, commentOnGithub, finalReportName);
      });
    }
