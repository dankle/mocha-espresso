...
var handleResult = function(rerunFailed, commentOnGitHub, finalJSONreportName) {
  ...
  if(rerunProcesses === 0) {
    if(results.indexOf(true) > -1){
      var finalReportPath = Path.join(reportDirectory, finalJSONreportName);
...
var runMochaProcess = function(file) {
  ...
  child.on('close', function(code) {
    ...
    if(code > 0) {
      ...
      Rerun.reRunMain(outputReportPath + '.xml', outputReportPath + '.json', false, 0, mochaTestFolder, reportDirectory);
      if(!debug) bar.tick(1);
