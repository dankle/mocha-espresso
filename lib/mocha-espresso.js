 ...
 *
 * node lib/mocha-espresso.js ../test/awesome_project/regression/ -r 2 -d -m "-g @smoke"
 *
 * Usage: run_all <mocha_test_folder> [-r <rerun_times>] [-d] [-m "<mocha_args ...>"]
 *
 * Options:
 *
...
var initSettings = function() {
  ...
  var configEnvPath = process.env.MOCHA_ESPRESSO_CONFIG;
  if(configEnvPath) {
    // Check path described in env variable
    Fs.stat(configEnvPath, function(err, stats) {
      if(err) {
        console.log('Path to config file described in env MOCHA_ESPRESSO_CONFIG not found, checking ./config');
        var configLocalPath = Path.join(__dirname, 'config', 'mocha-espresso-config.json');

        // Check main root for config file
        Fs.stat(configLocalPath, function(err, stats) {
          if(err) {
            console.log('Local mocha-espresso-config.json not found, using cli arguments');
            checkArguments();
          } else {
        ...
        var Config = require(configEnvPath);
        checkConfigFile(Config);
      }
    });
  } else {
    console.log('Environment variable MOCHA_ESPRESSO_CONFIG not found, using cli arguments');
    checkArguments();
  }
};

// Config for using mocha-esspresso with config file
var checkConfigFile = function (config) {
...
var checkArguments = function() {
  ...
  Program
      ...
      .option('-repo, --repo <repo>', 'Repo for github comment', '')
...
var parseResultFile = function(finalJSONreportName, passed, callback) {
  var json_path = Path.join(Config.report_directory, finalJSONreportName);
...
var runMochaProcess = function(file) {
  ...
  var multiArgument = 'multi=\'tap=- xunit=./test/reports/' + outputReportPath +
      '.xml json=./test/reports/' + outputReportPath + '.json\'';
  ...
  child.on('close', function(code) {
    ...
    if(code > 0) {
      ...
      Rerun.reRunMain(outputReportPath + '.xml', outputReportPath + '.json', true, rerun_times, mochaTestFolder, function(rerunFailed, finalJSONreportName){
        if(!debug) bar.tick(1);
...
var mochaTestFolder;
var reportDirectory = Path.join(__dirname, '..', 'test', 'reports');
