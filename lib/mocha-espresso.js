...
// Run test process
var runMochaProcess = function(file, globalNpmDir) {
  ...
  var globalMochaPath = Path.join(globalNpmDir, 'mocha-espresso/node_modules/mocha/bin/', 'mocha');
  var multiArgument = 'multi=\'tap=- xunit=' + reportDirectory + '/' + outputReportPath +
      '.xml json=' + reportDirectory + '/' + outputReportPath + '.json\'';

  var mochaCommand = multiArgument + ' ' + globalMochaPath + ' ' + mochaArguments +
      ' ' + file;
  ...
  child.on('close', function(code) {
    ...
    if(code > 0) {
      ...
      Rerun.reRunMain(outputReportPath + '.xml', outputReportPath + '.json', true, rerun_times, mochaTestFolder,
          globalMochaPath, reportDirectory, function(rerunFailed, finalReportName){
        if(!debug) bar.tick(1);
        handleResult(rerunFailed, commentOnGithub, finalReportName);
      });
    } else {
      Rerun.reRunMain(outputReportPath + '.xml', outputReportPath + '.json', false, 0, mochaTestFolder, globalMochaPath,
          reportDirectory, function(rerunFailed, finalReportName) {
        if (!debug) bar.tick(1);
        handleResult(rerunFailed, commentOnGithub, finalReportName);
      });
    }
...
var mochaArguments = '-R mocha-multi --recursive --slow 5000 --timeout 900000000';
var rerun_times = 2;
  ...
  initSettings(function() {
    ...
    filesInDirectory(function(err, files){
      ...
      Exec('npm root -g', function(error, stdout, stderr) {
        if(error) {
          console.log('Not able to get npm root folder: ' + error);
          process.exit(1);
        }
        // Get the global node_moduels dir
        var globalNpmDir = stdout.trim();

        // In some cases only one file exists in directory specified
        if(files instanceof Array){
          files.forEach(function(file) {
            runMochaProcess(file, globalNpmDir);
            rerunProcesses++;
          });
        } else {
          runMochaProcess(files, globalNpmDir);
          rerunProcesses++;
        }
