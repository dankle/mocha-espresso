...
var initSettings = function(callback) {
  ...
  if(configEnvPath) {
    ...
    Fs.stat(configEnvPath, function(err, stats) {
      if(err) {
        console.log('Path to config file described in env MOCHA_ESPRESSO_CONFIG not found, add valid path');
        process.exit(1);
      } else {
    ...
    Fs.stat(configLocalPath, function(err, stats) {
      if(err) {
        console.log('Local mocha-espresso.json not found, using cli arguments');
        checkArguments();
        return callback();
      } else {
...
var filesInDirectory = function(callback) {
  console.log('mochaTestFolder2: ' + mochaTestFolder);
...
var runMochaProcess = function(file) {
  ...
  console.log('outputreportpath: ' + outputReportPath);
